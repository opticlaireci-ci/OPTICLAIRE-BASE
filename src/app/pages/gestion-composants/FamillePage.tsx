...
interface Famille extends AuditInfo {
  id: string;
  famille: string;
}

function ModalFamille({ initial, onSave, onClose }: { initial?: Famille; onSave: (c: Famille) => void; onClose: () => void }) {
  const [form, setForm] = useState<Famille>(initial ?? {
  ...
  const doSave = () => {
    if (!form.famille) {
      alert('Veuillez remplir la famille');
      return;
    }
    onSave(form);
    onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.45)' }}>
      <div className="bg-white rounded shadow-2xl w-full max-w-md mx-4">
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 bg-gray-100">
          <span className="font-semibold text-gray-800">{initial ? 'Modifier' : 'Ajouter'} Famille</span>
          <button onClick={onClose} className="text-red-400 hover:text-red-600 font-bold text-lg px-1">×</button>
        </div>
        <div className="p-5 flex flex-col gap-4">
          <div>
            <label className={lCls}>Famille <span className="text-red-500">*</span></label>
            <input className={iCls} value={form.famille} onChange={e => setForm(f => ({ ...f, famille: e.target.value }))} />
          </div>
...
export function FamillePage() {
  const [familles, setFamilles] = useLiveData<Famille>('leclaire_db_familles');
  const [search, setSearch] = useState('');
  const [dateFilter, setDateFilter] = useState('');
  const [modal, setModal] = useState<{ mode: 'add' | 'edit'; item?: Famille } | null>(null);

  const filtered = useMemo(() => familles.filter(c => {
    if (search && !c.famille?.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  }), [familles, search, dateFilter]);

  const sorted = useMemo(() =>
    [...filtered].sort((a, b) => (a.famille || '').localeCompare(b.famille || '', 'fr')),
    [filtered]
  );

  const handleSave = (c: Famille) => {
    const isUpdate = familles.find(x => x.id === c.id);

    // Vérifier les doublons (seulement pour les nouvelles familles)
    if (!isUpdate) {
      const duplicate = familles.find(x =>
        x.famille.toLowerCase().trim() === c.famille.toLowerCase().trim()
      );
      if (duplicate) {
        alert(`La famille "${c.famille}" existe déjà dans le système.`);
        return;
      }
    }

    const familleWithAudit = isUpdate ? addUpdateAudit(c) : addCreateAudit(c);
    const next = isUpdate
      ? familles.map(x => x.id === c.id ? familleWithAudit : x)
      : [familleWithAudit, ...familles];
    setFamilles(next);
  };

  const handleDelete = (id: string) => {
    if (!window.confirm('Supprimer cette famille ?')) return;
    const next = familles.filter(c => c.id !== id);
    setFamilles(next);
  };

  return (
    <div className="flex flex-col gap-5 p-4 md:p-6" style={{ backgroundColor: '#f9fafb', minHeight: '100vh' }}>
      {modal && <ModalFamille initial={modal.item} onSave={handleSave} onClose={() => setModal(null)} />}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-5">Familles ({familles.length})</h2>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-white border-b border-gray-200">
                ...
                <th className="text-left px-4 py-3 font-semibold text-gray-700">Famille</th>
            ...
            <tbody>
              {sorted.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-10 text-gray-400">Aucune famille</td></tr>
              ) : sorted.map((c, idx) => (
                <tr key={c.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2.5 text-gray-800">{c.famille}</td>
        ...
        <div className="md:hidden flex flex-col gap-2">
          {sorted.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 0', color: '#9ca3af' }}>Aucune famille</div>
          ) : sorted.map(c => (
            <div key={c.id} style={{
