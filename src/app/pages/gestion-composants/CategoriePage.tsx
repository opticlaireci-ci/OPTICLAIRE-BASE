...
interface Categorie extends AuditInfo {
  id: string;
  categorie: string;
}

function ModalCategorie({ initial, onSave, onClose }: { initial?: Categorie; onSave: (c: Categorie) => void; onClose: () => void }) {
  const [form, setForm] = useState<Categorie>(initial ?? {
  ...
  const doSave = () => {
    if (!form.categorie) {
      alert('Veuillez remplir la catégorie');
      return;
    }
    onSave(form);
    onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.45)' }}>
      <div className="bg-white rounded shadow-2xl w-full max-w-md mx-4">
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 bg-gray-100">
          <span className="font-semibold text-gray-800">{initial ? 'Modifier' : 'Ajouter'} Catégorie</span>
          <button onClick={onClose} className="text-red-400 hover:text-red-600 font-bold text-lg px-1">×</button>
        </div>
        <div className="p-5 flex flex-col gap-4">
          <div>
            <label className={lCls}>Catégorie <span className="text-red-500">*</span></label>
            <input className={iCls} value={form.categorie} onChange={e => setForm(f => ({ ...f, categorie: e.target.value }))} />
          </div>
...
export function CategoriePage() {
  const [categories, setCategories] = useLiveData<Categorie>('leclaire_db_categories');
  const [search, setSearch] = useState('');
  const [dateFilter, setDateFilter] = useState('');
  const [modal, setModal] = useState<{ mode: 'add' | 'edit'; item?: Categorie } | null>(null);

  const filtered = useMemo(() => categories.filter(c => {
    if (search && !c.categorie?.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  }), [categories, search, dateFilter]);

  const sorted = useMemo(() =>
    [...filtered].sort((a, b) => (a.categorie || '').localeCompare(b.categorie || '', 'fr')),
    [filtered]
  );

  const handleSave = (c: Categorie) => {
    const isUpdate = categories.find(x => x.id === c.id);

    // Vérifier les doublons (seulement pour les nouvelles catégories)
    if (!isUpdate) {
      const duplicate = categories.find(x =>
        x.categorie.toLowerCase().trim() === c.categorie.toLowerCase().trim()
      );
      if (duplicate) {
        alert(`La catégorie "${c.categorie}" existe déjà dans le système.`);
        return;
      }
    }

    const catWithAudit = isUpdate ? addUpdateAudit(c) : addCreateAudit(c);
    const next = isUpdate
      ? categories.map(x => x.id === c.id ? catWithAudit : x)
      : [catWithAudit, ...categories];
    setCategories(next);
  };
  ...
  return (
    <div className="flex flex-col gap-5 p-4 md:p-6" style={{ backgroundColor: '#f9fafb', minHeight: '100vh' }}>
      {modal && <ModalCategorie initial={modal.item} onSave={handleSave} onClose={() => setModal(null)} />}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-5">Catégories ({categories.length})</h2>
        ...
        <div className="hidden md:block border border-gray-200 rounded overflow-hidden">
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {sorted.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-10 text-gray-400">Aucune catégorie</td></tr>
              ) : sorted.map((c, idx) => (
                <tr key={c.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2.5 text-gray-800">{c.categorie}</td>
        ...
        <div className="md:hidden flex flex-col gap-2">
          {sorted.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 0', color: '#9ca3af' }}>Aucune catégorie</div>
          ) : sorted.map(c => (
            <div key={c.id} style={{
