...
interface Matiere extends AuditInfo {
  id: string;
  matiere: string;
}

function ModalMatiere({ initial, onSave, onClose }: { initial?: Matiere; onSave: (c: Matiere) => void; onClose: () => void }) {
  const [form, setForm] = useState<Matiere>(initial ?? {
  ...
  const doSave = () => {
    if (!form.matiere) {
      alert('Veuillez remplir la matière');
      return;
    }
    onSave(form);
    onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.45)' }}>
      <div className="bg-white rounded shadow-2xl w-full max-w-md mx-4">
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 bg-gray-100">
          <span className="font-semibold text-gray-800">{initial ? 'Modifier' : 'Ajouter'} Matière</span>
        ...
        <div className="p-5 flex flex-col gap-4">
          <div>
            ...
            <input className={iCls} value={form.matiere} onChange={e => setForm(f => ({ ...f, matiere: e.target.value }))} />
          </div>
...
export function MatierePage() {
  const [matieres, setMatieres] = useLiveData<Matiere>('leclaire_db_matieres');
  const [search, setSearch] = useState('');
  const [dateFilter, setDateFilter] = useState('');
  const [modal, setModal] = useState<{ mode: 'add' | 'edit'; item?: Matiere } | null>(null);

  const filtered = useMemo(() => matieres.filter(c => {
    if (search && !c.matiere?.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  }), [matieres, search, dateFilter]);

  const handleSave = (c: Matiere) => {
    const isUpdate = matieres.find(x => x.id === c.id);
    const matiereWithAudit = isUpdate ? addUpdateAudit(c) : addCreateAudit(c);
    const next = isUpdate
      ? matieres.map(x => x.id === c.id ? matiereWithAudit : x)
      : [matiereWithAudit, ...matieres];
    setMatieres(next);
  };

  const handleDelete = (id: string) => {
    if (!window.confirm('Supprimer cette matière ?')) return;
    const next = matieres.filter(c => c.id !== id);
    setMatieres(next);
  };

  return (
    <div className="flex flex-col gap-5 p-6" style={{ backgroundColor: '#f9fafb', minHeight: '100vh' }}>
      {modal && <ModalMatiere initial={modal.item} onSave={handleSave} onClose={() => setModal(null)} />}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-5">Matières ({matieres.length})</h2>
        ...
        <div className="border border-gray-200 rounded overflow-hidden">
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-10 text-gray-400">Aucune matière</td></tr>
              ) : [...filtered].sort((a, b) => (a.matiere || '').localeCompare(b.matiere || '', 'fr')).map((c, idx) => (
                <tr key={c.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2.5 text-gray-800">{c.matiere}</td>
