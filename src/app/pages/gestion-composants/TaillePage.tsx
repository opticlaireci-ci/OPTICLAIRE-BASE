...
interface Taille extends AuditInfo {
  id: string;
  taille: string;
}

function ModalTaille({ initial, onSave, onClose }: { initial?: Taille; onSave: (c: Taille) => void; onClose: () => void }) {
  const [form, setForm] = useState<Taille>(initial ?? {
  ...
  const doSave = () => {
    if (!form.taille) {
      alert('Veuillez remplir la taille');
      return;
    }
    onSave(form);
    onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.45)' }}>
      <div className="bg-white rounded shadow-2xl w-full max-w-md mx-4">
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 bg-gray-100">
          <span className="font-semibold text-gray-800">{initial ? 'Modifier' : 'Ajouter'} Taille</span>
          <button onClick={onClose} className="text-red-400 hover:text-red-600 font-bold text-lg px-1">×</button>
        </div>
        <div className="p-5 flex flex-col gap-4">
          <div>
            <label className={lCls}>Taille <span className="text-red-500">*</span></label>
            <input className={iCls} value={form.taille} onChange={e => setForm(f => ({ ...f, taille: e.target.value }))} />
          </div>
...
export function TaillePage() {
  const [tailles, setTailles] = useLiveData<Taille>('leclaire_db_tailles');
  const [search, setSearch] = useState('');
  const [dateFilter, setDateFilter] = useState('');
  const [modal, setModal] = useState<{ mode: 'add' | 'edit'; item?: Taille } | null>(null);

  const filtered = useMemo(() => tailles.filter(c => {
    if (search && !c.taille?.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  }), [tailles, search, dateFilter]);

  const sorted = useMemo(() =>
    [...filtered].sort((a, b) => (a.taille || '').localeCompare(b.taille || '', 'fr')),
    [filtered]
  );

  const handleSave = (c: Taille) => {
    const isUpdate = tailles.find(x => x.id === c.id);

    // Vérifier les doublons (seulement pour les nouvelles tailles)
    if (!isUpdate) {
      const duplicate = tailles.find(x =>
        x.taille.toLowerCase().trim() === c.taille.toLowerCase().trim()
      );
      if (duplicate) {
        alert(`La taille "${c.taille}" existe déjà dans le système.`);
        return;
      }
    }

    const tailleWithAudit = isUpdate ? addUpdateAudit(c) : addCreateAudit(c);
    const next = isUpdate
      ? tailles.map(x => x.id === c.id ? tailleWithAudit : x)
      : [tailleWithAudit, ...tailles];
    setTailles(next);
  };

  const handleDelete = (id: string) => {
    if (!window.confirm('Supprimer cette taille ?')) return;
    const next = tailles.filter(c => c.id !== id);
    setTailles(next);
  };

  return (
    <div className="flex flex-col gap-5 p-4 md:p-6" style={{ backgroundColor: '#f9fafb', minHeight: '100vh' }}>
      {modal && <ModalTaille initial={modal.item} onSave={handleSave} onClose={() => setModal(null)} />}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-5">Tailles ({tailles.length})</h2>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-white border-b border-gray-200">
                ...
                <th className="text-left px-4 py-3 font-semibold text-gray-700">Taille</th>
            ...
            <tbody>
              {sorted.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-10 text-gray-400">Aucune taille</td></tr>
              ) : sorted.map((c, idx) => (
                <tr key={c.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2.5 text-gray-800">{c.taille}</td>
        ...
        <div className="md:hidden flex flex-col gap-2">
          {sorted.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 0', color: '#9ca3af' }}>Aucune taille</div>
          ) : sorted.map(c => (
            <div key={c.id} style={{
