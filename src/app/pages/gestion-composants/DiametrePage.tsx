...
interface Diametre extends AuditInfo {
  id: string;
  diametre: string;
}

function ModalDiametre({ initial, onSave, onClose }: { initial?: Diametre; onSave: (c: Diametre) => void; onClose: () => void }) {
  const [form, setForm] = useState<Diametre>(initial ?? {
  ...
  const doSave = () => {
    if (!form.diametre) {
      alert('Veuillez remplir le diamètre');
      return;
    }
    onSave(form);
    onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.45)' }}>
      <div className="bg-white rounded shadow-2xl w-full max-w-md mx-4">
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 bg-gray-100">
          <span className="font-semibold text-gray-800">{initial ? 'Modifier' : 'Ajouter'} Diamètre</span>
        ...
        <div className="p-5 flex flex-col gap-4">
          <div>
            ...
            <input className={iCls} value={form.diametre} onChange={e => setForm(f => ({ ...f, diametre: e.target.value }))} />
          </div>
...
export function DiametrePage() {
  const [diametres, setDiametres] = useLiveData<Diametre>('leclaire_db_diametres');
  const [search, setSearch] = useState('');
  const [dateFilter, setDateFilter] = useState('');
  const [modal, setModal] = useState<{ mode: 'add' | 'edit'; item?: Diametre } | null>(null);

  const filtered = useMemo(() => diametres.filter(c => {
    if (search && !c.diametre?.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  }), [diametres, search, dateFilter]);

  const handleSave = (c: Diametre) => {
    const isUpdate = diametres.find(x => x.id === c.id);
    const diametreWithAudit = isUpdate ? addUpdateAudit(c) : addCreateAudit(c);
    const next = isUpdate
      ? diametres.map(x => x.id === c.id ? diametreWithAudit : x)
      : [diametreWithAudit, ...diametres];
    setDiametres(next);
  };

  const handleDelete = (id: string) => {
    if (!window.confirm('Supprimer ce diamètre ?')) return;
    const next = diametres.filter(c => c.id !== id);
    setDiametres(next);
  };

  return (
    <div className="flex flex-col gap-5 p-6" style={{ backgroundColor: '#f9fafb', minHeight: '100vh' }}>
      {modal && <ModalDiametre initial={modal.item} onSave={handleSave} onClose={() => setModal(null)} />}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 className="text-lg font-semibold text-gray-800 mb-5">Diamètres ({diametres.length})</h2>
        ...
        <div className="flex items-center gap-3 mb-5">
          <div className="flex items-center border border-gray-300 rounded bg-white overflow-hidden" style={{ width: 380 }}>
            <input
              ...
              placeholder="Recherche Diamètre..."
              value={search}
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-white border-b border-gray-200">
                ...
                <th className="text-left px-4 py-3 font-semibold text-gray-700">Diamètre</th>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-10 text-gray-400">Aucun diamètre</td></tr>
              ) : [...filtered].sort((a, b) => (a.diametre || '').localeCompare(b.diametre || '', 'fr')).map((c, idx) => (
                <tr key={c.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2.5 text-gray-800">{c.diametre}</td>
